...
using TMPro;
using Photon.Pun;
using CJ; // Add this line to include the namespace containing the Projectile class
...
namespace CJ
{
    public class Handler_Movement_OnPlayer : MonoBehaviour
    {
        ...
        public float rangedAttackSpeed; // Speed of the ranged projectile
        public float rangedAttackDelay1; // Delay for first animation
        public float rangedAttackDelay2; // Delay for second animation
        #endregion
        // BASIC ATTACK
                ...
                { // Check cooldown
                    ...
                    { // Check range & if isAttackTarget
                        ...
                        if (isRangedClass)
                        {
                            ...
                            StartCoroutine(ShootProjectileAtTarget(isAttackTarget, projectileSpawnPoint, 1.2f, rangedAttackSpeed, delay));
                        ...
                        else
                        {
                            ...
                            StartCoroutine(WaitForImpact(float_WaitForImpact)); // Timing for impact VFX effect
        ...
        private IEnumerator ShootProjectileAtTarget(GameObject target, Transform spawnPoint, float hitHeight, float speed, float delay)
        {
            ...
            projectileScript.Initialize(target, Random.Range(5, 10), speed, spawnPoint.position.y, hitHeight, hitEffectPrefab); // Pass hitEffectPrefab
        ...
        private void OnHitTarget(GameObject target, float hitHeight)
        {
            target.GetComponent<Handler_Stats>().TakeDamage(Random.Range(5, 10));
        ...
        public void FBandage()
        {
            ...
            { // Movement state checks & Cooldown check
